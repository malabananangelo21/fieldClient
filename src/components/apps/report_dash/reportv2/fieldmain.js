...
function FieldReportv2() {
    ...
    getJOAuditFilterDashBoard(data).then((response) => {
      ...
      if (response.jobOrders.length != 0) {
        response.jobOrders.map((x) => {
          let match = findings.findIndex(
            (y) => String(y).toUpperCase() === String(x.findings).toUpperCase()
          );
          if (match === -1) {
            findings.push(x.findings);
          }
          if (String(x.findings).toUpperCase() === "DISCONNECTED") {
            let matchSub = disconFindings.findIndex(
              (y) =>
                String(y).toUpperCase() === String(x.installation).toUpperCase()
            );
            if (matchSub === -1) {
        ...
        let matchLogo = response.companies.findIndex(
          (y) => y.company_id === company_id
        );
        if (matchLogo > -1) {
    ...
    if (state.status === "ALL") {
      ...
      if (state.finding === "Disconnected") {
        if (state.sub_findings === "ALL") {
          ...
          filtered = refiltered.filter(
            (val) => val.installation === state.sub_findings
          );
        }
      } else {
        filtered = refiltered;
  ...
  const handleExportField = (fields) => {
    const currentIndex = state.exportCat.findIndex(
      (x) =>
        String(x.category_details).toUpperCase() ===
        String(fields.category_details).toUpperCase()
    );
    if (currentIndex === -1) {
  ...
  const handleChangeCustom = (val) => {
    const currentIndex = state.initialCat.findIndex(
      (x) =>
        String(x.category_details).toUpperCase() ===
        String(val.category_details).toUpperCase()
    );
    if (currentIndex === -1) {
      ...
      <Card variant="outlined">
        <CardContent>
          <Typography variant="h5">
            {" "}
            <b style={{ color: "#2980b9", marginBottom: 30 }}>
              Report's dashboard
            </b>
          </Typography>
          <Card variant="outlined">
