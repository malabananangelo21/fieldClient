...
function Schedule_Table() {
  ...
  const onFormSubmit = e => {
    e.preventDefault()
    if (moment(state.selection.startDate).format('YYYY-MM-DD') > moment(state.selection.endDate).format('YYYY-MM-DD')) {
      setState({
        ...state,
        alertError: true
      })
    } else if (moment(state.selection.startDate).format('YYYY') === moment('2020-01-01').format('YYYY')) {
      alert(
      ...
      setliquidChartArr_([])
      onSubmit(state.selection.startDate, state.selection.endDate, state.selected_branch, selected_jo_type, state.selected_ba, state.company)
      liquidFillAccomplishments(state.selected_branch, selected_jo_type[0], state.selection.startDate, state.selection.endDate, [state.selected_ba])
    }
  }
  ...
  const toggleSubmit = (fieldwork, ba, branch) => {
    ...
    setTimeout(() => {
      onSubmit(state.selection.startDate, state.selection.endDate, branch.branch_id, [fieldwork], ba, branch.company_id)
      liquidFillAccomplishments(branch.branch_id, fieldwork, state.selection.startDate, state.selection.endDate, [ba])
    }, 200)
  }


  const onSubmit = (startDate, endDate, selectedBranch, jo_type, selected_ba, company_id) => {
    let data = {
  ...
  const initialFetchAccomplishment = (branches) => {
    ...
    setTimeout(() => {
      onSubmit(curr_date, curr_date, BranchwithFieldWork[0].branch_id, [jo_type[parseInt(BranchwithFieldWork[0].default_jo_type)]], "ALL", BranchwithFieldWork[0].company_id)
  ...
  const onSubmitRefresh = () => {
    ...
    setliquidChartArr_([])
    setTimeout(() => {
      onSubmit(state.selection.startDate, state.selection.endDate, state.selected_branch, selected_jo_type, "ALL", state.company)
      liquidFillAccomplishments(state.selected_branch, selected_jo_type[0], state.selection.startDate, state.selection.endDate, ["ALL"])
    }, 200)
  }
  ...
  return (
    <div className={classes.root}>
      ...
      <Typography style={{ color: '#3973b6', fontWeight: 'bold', fontSize: 17 }}>Accomplishments</Typography>
        {state.selection.endDate !== '' ? (
          <Typography style={{ color: '#444b5a', fontWeight: 'bold', fontSize: 17 }}>
            Date Range:&nbsp;
      ...
      <Grid container spacing={3}>
        <Grid item md={9} style={{ display: 'flex', justifyContent: 'flex-start' }}>
          <Toggling handledBranch={home_reducer.handleBranch} toggleSubmit={toggleSubmit} />
          <Button
            size='small'
            variant='contained'
          ...
          <Button
            size='small'
            variant='contained'
            style={{ backgroundColor: 'rgba(6,86,147)', color: 'white', marginLeft: 10 }}
            className={classes.button}
          ...
          <Card variant="outlined">
            <CardContent style={{ height: 700 }}>
              {liquidChartArr_.map((value, index) => {
                ...
                return <div style={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center', height: 50, width: '100%' }} key={index}>
                  <div style={{ width: '85%' }}>
