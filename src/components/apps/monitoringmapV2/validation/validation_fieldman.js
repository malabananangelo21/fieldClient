...
const ValidatorFieldman = forwardRef((props, ref) => {
  ...
  const onChangeRemarksCategory = (e) => {
    setValidState((prev) => ({
      ...prev,
      validation_remarks_category: e.target.value,
    }));
  };
  ...
  const onEditData = (name) => {
    ...
    if (name === "Field Findings") {
      let data = {
        ...
        category_type:
          state.selectedDetails[0].accom_jo_type + " " + "Field Finding",
      };
  ...
  return (
    <div>
      {state.selectedDetails.map((val, index) => {
        ...
        let percent = parseInt(
          state.trackAccomMasterList.length * (parseInt(20) / 100)
        );
        if (state.trackAccomMasterList.length <= 50) {
          percent = state.trackAccomMasterList.length;
        }
        ...
        if (val.validator_remarks != "" && val.validator_remarks != null) {
                                ...
                                <form onSubmit={onSubmitRemarks}>
                                  ...
                                  "INVALID" ? (
                                    <>
                                      ...
                                      >
                                        {field_findings.map((val_findings) => {
                                          if (
                                            val_findings.status ===
                                            validState.validation_status
                                          )
                                      ...
                                      {validState.category_remarks.length >
                                      0 ? (
                                        <TextField
                                          required
                                          select
                                          ...
                                          style={{ width: "100%" }}
                                          value={
                                            validState.validation_remarks_category
                                          }
                                      ...
                                      <div>
                                        <TextareaAutosize
                                          onChange={(e) => {
                                            setValidState((prev) => ({
                                              ...prev,
                                              validator_comment: e.target.value,
                                            }));
                                          }}
                      ...
                      return (
                        <Grid item xs={12} md={6} key={index}>
                          <div style={{ position: "relative" }}>
                            ...
                            {validationState.editTextField ===
                              "Field Findings" &&
                            edit == true &&
                            val.name === "Field Findings" ? (
                              <TextField
                                required
                                select
                                size="small"
                              ...
                              >
                                {validationState.fieldFindings.map(
                                  (val, index) => {
                                    return (
                                      <MenuItem value={val.category_details}>
                            ...
                            {validationState.editTextField === val.name ? (
                              ...
                              >
                                <div
                                  ...
                                  className="loadermap2"
                                ></div>
                              </div>
                            ) : (
                              <EditIcon
                                onClick={() => onEditData(val.name)}
                                style={{
                                  display: edit_button,
                                  position: "absolute",
                                  right: 7,
                      ...
                      <div>
                        <Scrollbars style={{ height: 700 }}>
                          {trackAccom.map((val, index) => {
                            ...
                            let match_random =
                              state.index_randomJOToValidate.filter(
                                (val) => val == index
                              );
