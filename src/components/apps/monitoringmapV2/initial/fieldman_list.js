...
import regionList from "../../../../assets/json/refregion.json";
...
const AssigningField = ({ val, index, state, setState }) => {
  ...
  React.useEffect(() => {
    ...
    //             fetch("https://ph-locations-api.buonzz.com/v1/barangays")
    //               .then((response) => response.json())
    //               .then((data) => {
  ...
  const province = (e, index) => {
    let data = regionList.RECORDS.filter((val) => val.regCode === e);
    setRegionAddr(data[0].regDesc);
    let region = { region: data[0].regDesc };
    setState((prev) => ({
      ...prev,
    ...
    let province = provinceList.RECORDS.filter((val) => val.regCode == e);
  ...
  const city = (e, index) => {
    let data = provinceData.filter((val) => val.provCode === e);
    setProvinceAddr(data[0].provDesc);
    setState((prev) => ({
      ...prev,
      jo_type_assign: state.jo_type_assign.map((jo_data, index_jo) =>
        index === index_jo
    ...
    let cityDataList2 = cityList.RECORDS.filter((val) => val.provCode == e);
    setCity(cityDataList2);
  };

  const barangay = (e, index) => {
    let data = cityData.filter((val) => val.citymunCode === e);
    setCityAddr(data[0].citymunDesc);
  ...
  const brgy = (e, index) => {
    let data = barangayData.filter((val) => val.brgyCode === e);

    setBarangayAddr(data[0].brgyDesc);
    setState((prev) => ({
      ...prev,
      jo_type_assign: state.jo_type_assign.map((jo_data, index_jo) =>
        index === index_jo
        ...
        <Grid container spacing={1} key={index} style={{ marginTop: 25 }}>
          <Grid item xs={12} md={2}>
            ...
            >
              ...
              >
                {regionList.RECORDS.map((item) => (
                  <MenuItem key={item.regCode} value={item.regCode}>
                    {item.regDesc}
                  </MenuItem>
                ))}
              </Select>
              ...
              >
                {provinceData &&
                  ...
                  provinceData.map((item) => (
                    <MenuItem key={item.provCode} value={item.provCode}>
                      {item.provDesc}
                    </MenuItem>
                  ))}
              </Select>
            ...
            >
              ...
              >
                {cityData &&
                  cityData.length > 0 &&
                  cityData.map((item) => (
                    <MenuItem key={item.citymunCode} value={item.citymunCode}>
                      {item.citymunDesc}
                    </MenuItem>
                  ))}
              </Select>
            ...
            >
              ...
              >
                {barangayData.map((item) => (
                  <MenuItem key={item.brgyCode} value={item.brgyCode}>
                    {item.brgyDesc}
                  </MenuItem>
                ))}
              </Select>
...
const FieldmanList = forwardRef((props, ref) => {
  ...
  const province = (e, index) => {
    let data = regionList.filter((val) => val.regCode === e);
    setRegionAddr(data[0].regDesc);
    let region = { region: data[0].regDesc };
    setState((prev) => ({
      ...prev,
  ...
  const city = (e, index) => {
    let data = provinceData.filter((val) => val.id === e);
    setProvinceAddr(data[0].name);
    setState((prev) => ({
      ...prev,
      jo_type_assign: state.jo_type_assign.map((jo_data, index_jo) =>
        index === index_jo
