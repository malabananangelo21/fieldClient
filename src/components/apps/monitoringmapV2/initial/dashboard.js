...
const Home = forwardRef((props, ref) => {
  ...
  const Data_format = (data, data_filter) => {
    ...
    data_type = [
      {
        type: "SOA",
        count: formatNumber(jo_soa),
        assign: formatNumber(jo_assign_type.jo_soa),
        diff: jo_soa - jo_assign_type.jo_soa,
      },
      {
        type: "AUBD",
        count: formatNumber(jo_aubd),
        assign: formatNumber(jo_assign_type.jo_aubd),
        diff: jo_aubd - jo_assign_type.jo_aubd,
      },
      {
        type: "DN",
        count: formatNumber(jo_dn),
        assign: formatNumber(jo_assign_type.jo_dn),
        diff: jo_dn - jo_assign_type.jo_dn,
      },
      {
        type: "MECO",
        count: formatNumber(jo_meco),
        assign: formatNumber(jo_assign_type.jo_meco),
        diff: jo_meco - jo_assign_type.jo_meco,
      },
      {
        type: "NAC",
        count: formatNumber(jo_nac),
        assign: formatNumber(jo_assign_type.jo_nac),
        diff: jo_nac - jo_assign_type.jo_nac,
      },
      {
        type: "NCR",
        count: formatNumber(jo_ncr),
        assign: formatNumber(jo_assign_type.jo_ncr),
        diff: jo_ncr - jo_assign_type.jo_ncr,
      },
      {
        type: "OSB",
        count: formatNumber(jo_osb),
        assign: formatNumber(jo_assign_type.jo_osb),
        diff: jo_osb - jo_assign_type.jo_osb,
      },
      {
        type: "OSN",
        count: formatNumber(jo_osn),
        assign: formatNumber(jo_assign_type.jo_osn),
        diff: jo_osn - jo_assign_type.jo_osn,
      },
      {
        type: "REO DN",
        count: formatNumber(jo_dn_reout),
        assign: formatNumber(jo_assign_type.jo_dn_reout),
        diff: jo_dn_reout - jo_assign_type.jo_dn_reout,
      },
      {
        type: "REO SOA",
        count: formatNumber(jo_soa_reout),
    ...
    for (let index = 0; index < data.reout.length; index++) {
      ...
      if (!match) {
        if (element.type !== "REO ") {
          data_type.push({
            type: element.type,
            count: formatNumber(element.data.length),
            assign: formatNumber(assign_reout),
          });
        }
      }
    }
    ...
    let count_assign =
      ...
      jo_assign_type.jo_meco +
      jo_assign_type.jo_nac +
    ...
    data_type.unshift({
      type: "TODAY",
      count: formatNumber(count_receive),
      assign: formatNumber(count_assign),
      diff: formatNumber(count_receive - count_assign),
    });
    data_type.push({
      type: "TOTAL",
      count: formatNumber(total_jo_received),
      assign: formatNumber(total_assigned),
      diff: formatNumber(total_jo_received - total_assigned),
    });
    let total_accom = 0;
    let index_match = "";
    let last_accom_time = "";
    ...
    new_fieldman.map((val, index_val) => {
      ...
      if (val.attendance.length === 0) {
        ...
        let match_rescue = rescue_diff_position.filter(
          (val_rescue) => val_rescue.user_id === val.user_id
        );
      ...
      if (
        val.jo_accom > 0 &&
        val.attendance.length < 3 &&
        moment(first_attendance).format("YYYY-MM-DD") ===
    ...
    let last_fieldman_location = with_jo.filter(
      (val) =>
    ...
    onChangetrackAccom(
      ...
      excelFile,
      excel_invalid_data,
      data_filter.selection[0],
      data_filter.company_id,
      data.company
    );
    ...
    rescue_diff_position.forEach((val) => {
      if (val.attendance.length > 0) {
        ...
        if (
          moment(val.attendance[0].date_added).format("LT") >
          moment("2021-01-01 " + val.day_sched.attn_in).format("LT")
        ) {
    ...
    with_schedule.forEach((val) => {
      ...
      let absent_data = absent.filter(
        (val_absent) => val_absent.user_id === val.user_id
      );
    ...
    setState((prev) => ({
      ...
      rescue_data,
      get_lack_of_time: data.get_lack_of_time,
    }));
    handleClose();
  ...
  return (
    <div style={{ overflowY: "auto", height: "100%" }}>
                          ...
                          if (index == 0)
                            return (
                              ...
                              >
                                ...
                                &nbsp;
                                {map_reducer.selected_jo.length > 1 ? (
                                  <Typography
                                    key={index}
                                    className="font-color-white"
                                    style={{
                        ...
                        onClick={(e) => {
                          let data = state.fieldman.filter(
                            (val, index) =>
                              ...
                              String(
                                map_reducer.job_position
                              ).toLocaleUpperCase() ===
                                String(val.user_jobposition).toLocaleUpperCase()
                          );
                          //  onClickFieldman(data, state.date_start_val,'',state.get_lack_of_time);
                          onClickFieldman(
                            data,
                            state.date_start_val,
                            "",
                      ...
                      >
                        ...
                        >
                          {state.fieldman.reduce((count, val) => {
                            ...
                            parseInt(
                              state.array_dashboard_data.reduce(
                                (count, val) => {
                                  if (val.type === "RESCUE") {
                                    count = val.data.length;
                                  }
              ...
              <Card
                ...
                onClick={(e) => {
                  let data = state.fieldman.filter(
                    (val, index) =>
                      parseInt(val.user_delete_id) === 0 &&
                      val.employee_status === "Probationary" &&
                      val.jo_sched !== "Rescue"
                  );
                  setState((prev) => ({ ...prev, breakdown: false }));
                  onClickFieldman(
              ...
              <Card
                ...
                onClick={(e) => {
                  let data = state.fieldman.filter(
                    (val, index) =>
                      parseInt(val.user_delete_id) === 0 &&
                      val.employee_status === "For Orientation" &&
                      val.jo_sched !== "Rescue"
                  );
                  setState((prev) => ({ ...prev, breakdown: false }));
                  onClickFieldman(
            ...
            <Grid item xs={12} md={4}>
              <Card elevation={4} className="breakdown">
                <CardContent>
                  ...
                  >
                    SCHEDULED : {state.with_schedule.length}
                  </Typography>
                  <div
              ...
              <Card elevation={4} className="breakdown">
                <CardContent>
                  <Typography
                    style={{ color: "#fff", fontSize: 15, fontWeight: "bold" }}
                  >
                    NO SCHEDULED : {state.no_schedule.length}
                  </Typography>
                  <div
                    style={{ paddingLeft: 20, paddingRight: 20, marginTop: 10 }}
                  >
                        ...
                        <TableRow>
                          <MuiTableCell>
                            ...
                            >
                              ...
                              <Typography
                                style={{ color: "#333232", fontSize: 12.8 }}
                              >
                                {"test"}
                              </Typography>
                            </div>
                  ...
                  return (
                    <div>
                      <Typography style={{ fontSize: 14, fontWeight: "bold" }}>
                        {val} :{" "}
                        {state.selectedBreakdownData.reduce((count, val2) => {
                          if (
                            String(val).toUpperCase() ===
                            String(val2.employee_status).toUpperCase()
                          ) {
                            count++;
                          }
                          return count;
      ...
      >
        ...
        <DialogContent>
          <TrackingAssignedLocation
            state={state}
            setState={setState}
            fieldman_map={fieldman_map}
          />
        </DialogContent>
      </Dialog>
      <Dialog
