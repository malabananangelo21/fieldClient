  ...
  if (active && payload && payload.length && payload[0].payload.count !== 0 && customActive) {
    return (
      <div className="custom-tooltip" style={{ backgroundColor: '#fff', paddingRight: 10, paddingLeft: 10, paddingTop: 5, paddingBottom: 5, paddingLeft: 10, borderRadius: 5 }}>
        ...
        <p className="label" style={{ fontSize: 16, color: '#f1c40f',display:state.display_ave_accom?undefined:"none",fontWeight: 'bold' }}>ACCOMPLISHED  : <span style={{ fontWeight: 'bold' }}>{`${payload[0].payload.average_accom}`}</span></p>
        <p className="label" style={{ fontSize: 16, color: '#3498db',display:state.display_ave_assign?undefined:"none",fontWeight: 'bold'  }}>ASSIGNED  : <span style={{ fontWeight: 'bold' }}>{`${payload[0].payload.average_assign}`}</span></p>
