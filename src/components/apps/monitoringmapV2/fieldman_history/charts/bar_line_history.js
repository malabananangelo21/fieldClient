...
const CustomTooltip = ({ active, payload, label, checkState }) => {
  if (active && payload && payload.length) {
    return (
      <div className="custom-tooltip card-color-data2" style={{ backgroundColor: '#fff', paddingRight: 10, paddingLeft: 10, paddingTop: 5, paddingBottom: 5, paddingLeft: 10, borderRadius: 5, width: 400 }}>
          ...
          <Grid item xs={12} md={12}>
            <div style={{ display: 'flex' }}>
              ...
              {typeof payload[0].payload.type !== 'undefined' ?
                <div>
                  <p className="label" style={{ fontSize: 16, color: 'red', fontWeight: 'bold' }}>ABSENT</p>
                </div> : undefined
              }
            </div>
          </Grid>
          ...
          <Grid container spacing={1}>
            ...
            : undefined
              ...
              <Grid container spacing={1}>
                {checkState.accomplishment ?
                  <Grid item xs={4} md={4}>
                    <Card variant={"outlined"} style={{ backgroundColor: '#2bad62' }}>
                      <div style={{ padding: 10, display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
                        <p className="label" style={{ fontSize: 16, color: '#fff', fontWeight: 'bold' }}>Accom</p>
                ...
                <Grid item xs={4} md={4}>
                  <Card variant={"outlined"} style={{ backgroundColor: '#fff', paddingLeft: 10, paddingRight: 10, paddingBottom: 3 }}>
                    <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', height: 20 }}>
                      <p className="label" style={{ fontSize: 13.6, color: '#27ae60', fontWeight: 'bold' }}>VALID</p>
                      <p className="label" style={{ fontSize: 13.6, color: '#27ae60', fontWeight: 'bold' }}><span style={{ fontWeight: 'bold' }}>{`${formatNumber(payload[0].payload.valid)}`}</span></p>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', height: 20 }}>
                      <p className="label" style={{ fontSize: 13.6, color: '#d35400', fontWeight: 'bold' }}>INVALID</p>
                      <p className="label" style={{ fontSize: 13.6, color: '#d35400', fontWeight: 'bold' }}><span style={{ fontWeight: 'bold' }}>{`${formatNumber(payload[0].payload.invalid)}`}</span></p>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', height: 30 }}>
                      <p className="label" style={{ fontSize: 13.6, color: '#2c3e50', fontWeight: 'bold' }}>REMAINING</p>
                      <p className="label" style={{ fontSize: 13.6, color: '#2c3e50', fontWeight: 'bold' }}><span style={{ fontWeight: 'bold' }}>{`${formatNumber(payload[0].payload.count_to_validate_stack)}`}</span></p>
                    </div>
                  </Card>
                </Grid>
              </Grid>
                ...
                <Card  style={{background:'#faebaf'}}>
                  <div style={{ padding: 10 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <Typography style={{ fontSize: 12.5, fontWeight: 'bold' }}>Appeal Justification</Typography>
                      <Typography style={{ fontSize: 14, fontWeight: 'bold', color: payload[0].payload.appeal_status === "Approved" ? "#6ab04c" : "#e74c3c" }}>{payload[0].payload.appeal_status}</Typography>

                    </div>

                    <Card variant='outlined' style={{background:'#faf0ca',padding:5}}>
                     
                        <Typography style={{ fontSize: 12.5, fontWeight: 'bold', marginTop: 5 }}>Approver</Typography>
                        {payload[0].payload.approver_hierarchy.map((val,index) => {
                           let payroll_status = 'PENDING'
                           if (parseInt(payload[0].payload.req_hierarchy_level) > index) {
                             payroll_status = "APPROVED"
                           }
                          return <div style={{ marginLeft: 15 }}>
                            {val.approver.map((approver, index_approver) => {
                             
                              return <Typography style={{ fontSize: 12.5, marginTop: 5, fontWeight: 'bold' }}>{approver.name} <span style={{ marginLeft: 10,color:payroll_status == "APPROVED"?"#6ab04c":"#e74c3c" }}>{payroll_status}</span></Typography>
                            })
                            }
                          </div>
                        })
                    ...
                    <Card variant='outlined' style={{background:'#faf0ca',marginTop:5,padding:5}}>
               
                    <Typography style={{ fontSize: 12.5, fontWeight: 'bold', marginTop: 5 }}>Appeal Percentage : {payload[0].payload.approved_hours}%</Typography>
...
const CustomizedDot = (props) => {
  ...
  if (((payload.accomplishment < payload.total_jo * 0.95) && !payload.appeal_requests) || (!payload.attendance && !payload.appeal_requests)) {
    match_absent = true
  }
  if (typeof payload.type !== 'undefined' || match_absent) {
    return (
      <svg xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1" viewBox="0 0 24 24" x={cx - 10} y={cy - 10} width={23} height={23} fill="#f39c12"><path d="M24,10V24H0V10Zm0-2V5a3,3,0,0,0-3-3H18V0H16V2H8V0H6V2H3A3,3,0,0,0,0,5V8Zm-6,6H16v2h2Zm-5,0H11v2h2ZM8,14H6v2H8Zm10,4H16v2h2Zm-5,0H11v2h2ZM8,18H6v2H8Z" /></svg>
...
export default function Barline2({ checkState, summary, dataFilter, company_id, jo_type, getdateDetails }) {
    ...
    <div style={{ width: '100%', height: '100%' }}>
      ...
      <ResponsiveContainer>
        ...
        >
          ...
          <YAxis tick={{ fill: '#fff' }} />
          <Tooltip position={{ y: -250 }} content={<CustomTooltip checkState={checkState} />} />
          ...
          {checkState.accomplishment ?
            <Line strokeWidth={3} type="monotone" dataKey="accomplishment" stroke="#2bad62" />
            : undefined
          }
          {checkState.remaining ?
            <Line strokeWidth={3} type="monotone" dataKey="remaining" stroke="#e74c3c" dot={<CustomizedDot />} />
            : undefined
          }
        </ComposedChart>
      </ResponsiveContainer>
