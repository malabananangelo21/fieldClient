...
import { Button, Grid, IconButton, Typography, Card, CardContent, Dialog, DialogTitle, DialogContent } from "@material-ui/core";
...
export default function Dashboard({ setIndexState }) {
    ...
    const [state, setState] = React.useState({
        ...
        filter_date_end: new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0),
        job_order_type: "",
        type: '',
        array_dashboard: [
                {
                    type: 'TOTAL', value: 0, percentage: "", color: '#9b59b6'
                },
                {
                    type: 'BACKLOG LAST MONTH', value: 0, percentage: "", color: '#004987'
                },
                {
                    type: 'ACCOMPLISHED', value: 0, percentage: 0, color: '#2bad62'
                },
                {
                    type: 'UNDELIVERED', value: 0, percentage: 0, color: '#d44b3b'
                }],
        array_dashboard2: [
            {
                type: 'TOTAL RE-OUT', value: 0, percentage: '', color: ''
            },
            {
                type: 'ASSIGNED RE-OUT', value: 0, percentage: 0, color: ''
            },
            {
                type: 'ACCOMPLISHED RE-OUT', value: 0, percentage: 0, color: ''
            },
            {
                type: 'UNDELIVERED RE-OUT', value: 0, percentage: 0, color: ''
            },
            {
                type: 'AVE. ASSIGN PER FIELDMAN', value:0, percentage: '', color: ''
            },
            {
                type: 'AVE. ACCOM PER DAY', value: 0, percentage: '', color: ''
            },
        ],
        branch_name: '',
        jo_type: ""
    ...
    const getBranches = React.useCallback(async function getBranches() {
        ...
        await getData(
            ...
            if (onSelectData !== null) {
                ...
                get_accom_report(state.filter_date_start, state.filter_date_end, branch_id, company_id, branch_name, data_route.jo_type)
            }
            // else{
    ...
    const get_accom_report = (filter_date_start, filter_date_end, bid, c_id, b_name, jo_type_val) => {
        ...
        getData("tracking/summary_report_accom_beta_beta_test", data).then(res => {
            ...
            let array_dashboard = [
                {
                    type: 'TOTAL', value: total, percentage: "", color: '#9b59b6'
                },
                {
                    type: 'BACKLOG LAST MONTH', value: backlog_last_month, percentage: "", color: '#004987'
                },
                {
                    type: 'ACCOMPLISHED', value: accom, percentage: (accom / total) * 100, color: '#2bad62'
                },
                {
                    type: 'UNDELIVERED', value: (backlog_last_month+total) - accom, percentage: ((backlog_last_month+total) - accom) < 0?0:(((backlog_last_month+total) - accom) / total) * 100, color: '#d44b3b'
                }]
            let array_dashboard2 = [
                {
                    type: 'TOTAL RE-OUT', value: total_reout, percentage: '', color: ''
                },
                {
                    type: 'ASSIGNED RE-OUT', value: assigned_reout, percentage: (assigned_reout / total_reout) * 100, color: ''
                },
                {
                    type: 'ACCOMPLISHED RE-OUT', value: re_out_accom, percentage: (re_out_accom / total_reout) * 100, color: ''
                },
                // {
                //     type: 'UNASSIGNED RE-OUT', value: unassigned_reout, percentage: (unassigned_reout / total_reout) * 100, color: ''
                // },
                {
                    type: 'UNDELIVERED RE-OUT', value: total_reout - re_out_accom, percentage: ((total_reout - re_out_accom) / total_reout) * 100, color: ''
                },
                {
                    type: 'AVE. ASSIGN PER FIELDMAN', value: assigned / fieldman, percentage: '', color: ''
                },
                {
                    type: 'AVE. ACCOM PER DAY', value: accom / res.count_accom, percentage: '', color: ''
                },
            ]
            var date = new Date(filter_date_end);
            var lastDay = filter_date_end;
            let new_accom = res.new_accom
            if (moment(filter_date_start).format('MM') === moment(lastDay).format('MM')) {
            ...
            setState({ ...state, count_accom: res.count_accom, filter: false, branch_name: b_name, summary: res.new_accom, array_dashboard: array_dashboard, show_bar: true, filter_date_start: filter_date_start, filter_date_end: filter_date_end, branch_id: bid, company_id: c_id, jo_type: jo_type_val, type: '', array_dashboard2: array_dashboard2 });
            setIndexState(prev => ({ ...prev, dataFilter: data, summary: res.new_accom, company_id: c_id, jo_type: jo_type_val }))
        });
    };
    ...
    return (
        <div className='dashboard-summary ' >
            <Grid container spacing={1}>
                <Grid item xs={12} md={12}>
                    <div style={{ display: 'flex' }}>
                        ...
                        <Card className='card-color-data-summary' style={{ padding: 5, marginRight: 5 }}>
                            <Typography style={{ color: '#fff' }}>{state.branch_name}</Typography>
                        </Card>
                        <Card className='card-color-data-summary' style={{ padding: 5, marginRight: 5 }}>
                            <Typography style={{ color: '#fff' }}>{moment(state.filter_date_start).format('LL') + ' - ' + moment(state.filter_date_end).format('LL')}</Typography>
                        </Card>
                    </div>
                </Grid>
                {state.array_dashboard.map((val, index) => {
                    return <Grid item xs={12} md={2} key={index}>
                        <Card variant='outlined' style={{ padding: 10, backgroundColor: val.color, opacity: 0.9 }}>
                            <Grid container spacing={0}>
                                <Grid item xs={12} md={12}>
                                    <Typography className='text-color-white' style={{ fontSize: 13}}>{val.type}</Typography>
                                </Grid>
                                <Grid item xs={12} md={12}>
                                    <Typography className='text-color-white' style={{ fontSize: 25, textAlign: 'center' }}>{formatNumber(val.value)}</Typography>
                                    <Typography style={{ fontSize: 17, textAlign: 'center', color: val.type !== 'TOTAL' && val.type !=='BACKLOG LAST MONTH'? '#fff' : val.color, fontWeight: 'bold' }}>{isNaN(val.percentage) ? 0 : parseFloat(
                                        val.percentage).toFixed(2)}%</Typography>
                                </Grid>
                            </Grid>
                 ...
                 {state.array_dashboard2.map((val, index) => {  
                     if(val.type === "AVE. ASSIGN PER FIELDMAN" || val.type === "AVE. ACCOM PER DAY")
                    return <Grid item xs={12} md={2}>
                        <Card key={index} className='card-color-data-summary' style={{ height: 70, padding: 10, marginRight: 8 }}>
                            <div>
                                <Typography style={{ textAlign: 'left', color: '#fff', fontSize: 12.5, fontWeight: 'bold' }}>{val.type}</Typography>
                                <Typography style={{ textAlign: 'center', color: '#fff', fontSize: 20, fontWeight: 'bold' }}>{isNaN(val.value) ? 0 : formatNumber(parseInt(val.value))}</Typography>
                                <Typography style={{ textAlign: 'center', color: '#fff', fontSize: 15 }}>{val.percentage !== "" ? isNaN(val.percentage) ? 0 : parseFloat(val.percentage).toFixed(2) + "%" : undefined}</Typography>
                            </div>
                        </Card>
                    </Grid>
                })
            ...
            >
                ...
                <DialogContent>
                    <Filter company_id={state.company_id} branch_id={state.branch_id} filter_date_start={state.filter_date_start} filter_date_end={state.filter_date_end} get_accom_report={get_accom_report} closeModal={() => closeModal()} job_order_type={state.job_order_type} jo_type={state.jo_type} onReset={() => setState({ ...state, type: '' })} />
                </DialogContent>
            </Dialog>
        </div>
    );
