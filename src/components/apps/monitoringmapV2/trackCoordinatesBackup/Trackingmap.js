...
export default function GoogleMapContainer() {
    ...
    const accom_selected_jo = useSelector((state) => state.map_reducer.accom_selected_jo);
    ...
    const fetchAllAccomplishments = (data, branch_name, personName) => {
        ...
        getData("tracking/fetchAllAccomplishment", data).then((res) => {
            ...
            getImagesInitial(accom, 0, accom2, fieldman_list, fieldman_checked, branch_name, data, 'initial');
        });
    }
    const getImagesInitial = (jo_accom, index, accom2, fieldman_list, fieldman_checked, branch_name, data, type) => {
    ...
    const onCheckFieldman = (user_id) => {
        ...
        let new_marker = []
        console.log(state.fieldman_checked)
                            ...
                            <CardContent>
                                ...
                                <Grid container spacing={1}>
                                    <Grid item xs={12} md={12} style={{ display: state.fieldman_list.length == 0 ? 'none' : undefined }}>
                                        <div onClick={() => onCheckFieldman("ALL")} variant={"outlined"} style={{ padding: 5, cursor: 'pointer' }} >
                                            <Grid container spacing={1}>
                                ...
                                <div style={{ maxHeight: height_window * .57, overflowY: 'auto', overflowX: 'hidden', background: '#cfd2d4', paddingLeft: 5, paddingRight: 5, paddingTop: 5 }}>
                                    <Grid container spacing={1}>
                                                ...
                                                <Card variant={"outlined"} style={{ padding: 5, cursor: 'pointer' }} onClick={() => onCheckFieldman(val.user_id)}>
                                                    <Grid container spacing={1}>
                                                        ...
                                                        <Grid container item xs={2} md={2} justify="flex-end" alignContent="center">
                                                            <EventNoteIcon style={{ color: '#115293', width: 18, height: 18, display: state.selected_fieldman.length === 0 ? undefined : 'none' }} onClick={(e) => {
                                                                e.stopPropagation();
                                                                let match = []
                                                        ...
                                                        <Grid item xs={12} md={8} alignContent="center">
                                                            {state.location_description !== "" ? <Typography style={{ fontSize: 12, fontWeight: 'Bold' }}>ACCOM : {val.accom}</Typography> :
                                                                <Typography style={{ fontSize: 12, fontWeight: 'Bold' }}>ACCOM : {val.accom} / {val.jo_count}</Typography>
                                                            }
